...
module "alb" {
  ...
  allow_cidr_blocks = each.value.internal ? lookup(lookup(lookup(lookup(var.vpc , each.value.vpc_name , null), "private_subnets" , null),"web", null), "cidr_block", null) : [ "0.0.0.0/0"]
  vpc_id = lookup(lookup(module.vpc , each.value.vpc_name , null), "vpc_id" , null)
